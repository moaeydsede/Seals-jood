...
function showAppUI(isSignedIn){
  ...
  // show/hide sections except login
  if(isSignedIn){
    loginView.style.display = 'none';
    // enable navbar links + show default view
    nav.style.visibility = 'visible';
    switchView('viewCompany');
  }else{
    // show login full screen
    loginView.style.display = 'block';
    // keep navbar visible but minimal; user can open setup
    nav.style.visibility = 'visible';
    // hide other views
    document.querySelectorAll('.view').forEach(v => {
      if(v.id !== 'viewLogin') v.classList.add('d-none');
    });
  }
  // update title in login
...
$('#btnOpenSetupFromLogin')?.addEventListener('click', ()=>{ $('#btnOpenSetup').click(); });

$('#btnOpenSetup').addEventListener('click', ()=>{
  const cfg = F.loadSavedConfig();
...
$('#btnDoLogin').addEventListener('click', async ()=>{
  if(!F.Firebase.ready) return toast('أولاً: إعداد Firebase', 'warning');
  ...
  try{
    ...
    toast('فشل الدخول: تحقق من Email/Password', 'danger');
  }
});
...
$('#btnDoRegister').addEventListener('click', async ()=>{
  ...
  try{
    ...
    toast('فشل إنشاء الحساب: ' + (e?.message || ''), 'danger');
  }
});
...
function ensureLogoutBtn(){
  ...
  const host = document.querySelector('nav .d-flex.gap-2.align-items-center');
  if(!host) return;
  if(!btn){
    btn = document.createElement('button');
    btn.id = 'btnLogout';
    btn.className = 'btn btn-sm btn-outline-light';
    btn.innerHTML = '<i class="fa-solid fa-right-from-bracket me-1"></i>خروج';
...
function openInvoiceForEdit(inv){
  ...
  $('#invoicePermHint').textContent = 'ملاحظة: تعديل/حذف الفواتير للأدمن فقط.';
  renderInvoiceLines();
  invoiceModal.show();
}
...
function renderInvoicePrint(inv){
  ...
  const salesPhone = company.salesPhone2 || company.salesPhone1 || '';
  const socials = Array.isArray(company.socials) ? company.socials : [];

  // pick first matching urls
  const pickUrl = (k1,k2) => socials.find(u => (u||'').toLowerCase().includes(k1) || (k2 && (u||'').toLowerCase().includes(k2))) || '';
  const waUrl = pickUrl('wa.me','whatsapp');
  const tgUrl = pickUrl('t.me','telegram');
  const fbUrl = pickUrl('facebook','fb');
  const igUrl = pickUrl('instagram','insta');
  ...
  // like sample: "رصيد العميل قبل الفاتورة" + "رصيد العميل" بعد الفاتورة
  ...
  const rowsHtml = lines.map(l=>`
    <tr>
      ...
      <td class="mono">${escapeHtml(l.unit||'')}</td>
      <td class="mono">${fmt(l.price)}</td>
      <td class="mono">${fmt(l.total)}</td>
    </tr>
  `).join('');

  const totalWords = arabicMoneyWords(total, 'جنيه', 'قرش');
  ...
  $('printArea').innerHTML = `
    <div class="print-page">
      <div class="print-header">
        ...
        <div class="right">
          <div class="contact-title">Contact Sales</div>
          <div class="contact-icons">
            ${igUrl ? `<div class="ic"><i class="fa-brands fa-instagram"></i><div class="num"> </div></div>` : ``}
            ${fbUrl ? `<div class="ic"><i class="fa-brands fa-facebook"></i><div class="num"> </div></div>` : ``}
            ${tgUrl ? `<div class="ic"><i class="fa-brands fa-telegram"></i><div class="num">${escapeHtml(salesPhone||'')}</div></div>` : ``}
            ${waUrl ? `<div class="ic"><i class="fa-brands fa-whatsapp"></i><div class="num"> </div></div>` : ``}
            ${(!waUrl && !tgUrl && !fbUrl && !igUrl) ? `<div class="ic"><i class="fa-solid fa-phone"></i><div class="num">${escapeHtml(salesPhone||'')}</div></div>` : ``}
          </div>
        </div>
      </div>
      ...
      <div class="print-meta">
        ...
        <div class="block" style="text-align:end">
          ...
          <div><b>البيان:</b> ${escapeHtml(inv.notes||'')}</div>
        </div>
      </div>
      ...
      <div class="print-summary">
        <div class="box">
          <div class="rowx head"><span>المجموع</span><span class="mono">${fmt(total)}</span></div>
        ...
        <div class="box">
          <div class="rowx head"><span>الرصيد</span><span></span></div>
      ...
      <div class="print-words">فقط ${escapeHtml(totalWords)} لا غير</div>

      <div class="print-balances">
        <div>رصيد العميل : <span class="mono">${fmt(balAfter)}</span></div>
        <div>رصيد العميل قبل الفاتورة : <span class="mono">${fmt(balBefore)}</span></div>
      </div>
  ...
  $('printArea').innerHTML = `
    <div class="print-page">
      ...
      <div class="box" style="border:1px solid #cfd6df;border-radius:12px;padding:10px;margin-top:10px;">
        ...
        <div class="rowx" style="display:flex;justify-content:space-between;"><span>ملاحظات:</span><span>${escapeHtml(v.notes||'—')}</span></div>
      </div>
...
function arabicMoneyWords(amount, currencyMain='جنيه', currencySub='قرش'){
  const n = Number(amount || 0);
  const whole = Math.floor(Math.abs(n));
  const frac = Math.round((Math.abs(n) - whole) * 100);

  const w = (x)=> arabicNumberWords(x);

  let s = w(whole) + " " + currencyMain;
  if(frac > 0) s += " و " + w(frac) + " " + currencySub;
  if(n < 0) s = "سالب " + s;
  return s;
}

// Arabic number to words (0..999,999,999)
function arabicNumberWords(num){
  num = Number(num || 0);
  if(num === 0) return "صفر";
  ...
  const groups = [
    { value: 1_000_000_000, name: "مليار" },
    { value: 1_000_000, name: "مليون" },
    { value: 1_000, name: "ألف" }
  ];
  ...
  for(const g of groups){
    if(n >= g.value){
      ...
      if(count === 1){
        ...
        parts.push("اثنان " + g.name);
      }else{
        parts.push(threeDigits(count) + " " + g.name);
      }
    }
  }
...
async function boot(){
  ...
  // if config exists but not logged in, show login screen
  showAppUI(false);
}
boot();
