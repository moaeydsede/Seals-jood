...
// __BOOT_GUARD__
try{ if(window.__diagLog) window.__diagLog('✅ app.js تم تحميله'); }catch(e){}

(async function(){
  try{
    if(window.__APP_BOOTSTRAPPED) return;
    if(window.__diagLog) window.__diagLog('ℹ️ محاولة تشغيل الواجهة الأساسية...');
    const v=document.getElementById('view');
    if(v && !v.innerHTML.trim()){
      v.innerHTML = `
        <div class="card" style="max-width:520px;margin:24px auto">
          <div class="card-h">تسجيل الدخول</div>
          <div class="muted">إذا لم تظهر شاشة الدخول تلقائياً، فهذا يعني أن هناك خطأ في تحميل Firebase أو أحد الملفات. افتح زر (تشخيص) لرؤية الخطأ.</div>
          <div style="height:12px"></div>
          <a class="btn primary" href="#/login">فتح شاشة الدخول</a>
        </div>`;
    }
    window.__APP_BOOTSTRAPPED = true;
  }catch(e){
    try{ if(window.__diagLog) window.__diagLog('❌ فشل تشغيل الواجهة الأساسية: '+(e.message||e)); }catch(_){}
  }
})();