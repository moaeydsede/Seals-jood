...
function setConnBadge(ok){
  ...
  if(ok){
    ...
    b.className = 'badge text-bg-warning';
  }
}
...
function wireAuth(){
  ...
  F.onAuthStateChanged(F.Firebase.auth, async (user)=>{
    ...
    if(user){
      ...
      $('#btnDoLogout').classList.remove('d-none');
      $('#btnLogin').innerHTML = '<i class="fa-solid fa-user me-1"></i>الحساب';
      toast('تم تسجيل الدخول ✅', 'success');
    }else{
      currentRole = "guest";
      $('#btnDoLogout').classList.add('d-none');
      $('#btnLogin').innerHTML = '<i class="fa-solid fa-right-to-bracket me-1"></i>دخول';
    }
    renderWhoami();
    await refreshAll();
  });
    ...
    <div class="print-page">
      ...
      <div class="print-summary">
        ...
        <div class="box">
          ...
          <div class="rowx"><span>ملاحظات:</span><span>${escapeHtml(inv.notes||'—')}</span></div>
        </div>
      </div>
