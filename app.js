...
// دخول للجلسة سواء من Firebase Auth أو وضع محلي (للطوارئ إذا لم يكن مستخدم Firebase مُنشأ بعد)
async function enterAppSession(user, isLocal=false){
  try{
    USER=user;
    if(!user){
    ...
    if(isLocal){
      // صلاحيات كاملة محليًا
      USERDOC={ role:"admin", perms:{} };
    }else{
      try{
    ...
    if(USERDOC && USERDOC.companyId){
      COMPANY_ID=USERDOC.companyId;
      localStorage.setItem("companyId", COMPANY_ID);
    }

    try{ await loadCompany(); }catch(e){ console.warn(e); }
    try{ await loadPeriod(); }catch(e){ console.warn(e); }
    try{ setCompanyMini(); }catch(e){}
    ...
    try{ await refreshAudit(); }catch(e){}
...
$("btnLogin").onclick=async ()=>{
  ...
  try{
    ...
    try{
      ...
      // وضع محلي للطوارئ: admin / Admin123 (أو Admin 123)
      const p = (pass||"").trim();
      const isAdminUser = ["admin","admin@erp.local"].includes(raw.toLowerCase()) || ["admin@erp.local"].includes(email.toLowerCase());
      const isAdminPass = (p==="Admin123" || p==="Admin 123" || p==="admin123" || p==="ADMIN123");
      if(isAdminUser && isAdminPass){
        LOCAL_MODE=true;
        await enterAppSession({ uid:"local-admin", email: (email||raw) }, true);
        return;
      }
...
$("btnLogout").onclick=()=>{ if(LOCAL_MODE){ LOCAL_MODE=false; USER=null; USERDOC=null; $("loginPage").style.display="flex"; $("shell").style.display="none"; } else { signOut(auth); } };

onAuthStateChanged(auth, async (user)=>{ await enterAppSession(user, false); });