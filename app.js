...
async function loadInvoices(type){
  ...
  snap.forEach(d=>{ 
    ...
    const obj=normalizeInvoice({id:d.id,...d.data()});
    if(!type || obj.type===type) list.push(obj);
  });
  return list;
}

// Backward compatible invoice normalization (older docs may use customer/supplier fields)
function normalizeInvoice(inv){
  try{
    const type=String(inv?.type||"");
    // partyId
    if(!inv.partyId){
      if(type==="sale"||type==="sale_return") inv.partyId = inv.customerId || inv.party || "cash";
      else if(type==="purchase"||type==="purchase_return") inv.partyId = inv.supplierId || inv.party || "sup";
    }
    // partyName
    if(!inv.partyName){
      inv.partyName = inv.customerName || inv.supplierName || "";
      if(!inv.partyName && inv.partyId){
        // try lookup in cached lists
        if(type==="sale"||type==="sale_return"){
          inv.partyName = (inv.partyId==="cash")?"عميل نقدي":(CUSTOMERS.find(c=>c.id===String(inv.partyId))?.name||"");
        }else if(type==="purchase"||type==="purchase_return"){
          inv.partyName = (SUPPLIERS.find(s=>s.id===String(inv.partyId))?.name||"مورد");
        }
      }
    }
  }catch(_){/* ignore */}
...
function displayParty(inv){
  ...
  return (
    ...
    inv?.customerName ||
    inv?.supplierName ||
    ((type==="sale"||type==="sale_return") ? (pid==="cash"?"عميل نقدي":(CUSTOMERS.find(c=>c.id===pid)?.name||""))
                                             : (SUPPLIERS.find(s=>s.id===pid)?.name||"مورد"))
  ) || "";
}
  ...
  <div class="pdoc invoice invoicePro">
    ...
    <div class="invPartyRow">
      <div class="invPartyRight">
        <div class="invPartyLine"><span>السيد:</span> <b>${safeText(displayParty(inv))}</b></div>
...
function invoiceReceiptHTMLFrom(inv){
  ...
  <div class="receipt">
    ...
    <div class="rSmall">الطرف: ${safeText(displayParty(inv))}</div>
...
function mountInvoiceMoves(){
  ...
  const load=async ()=>{
    ...
    if(q){
      list=list.filter(inv=>String(inv.no).includes(q) || String(displayParty(inv)).includes(q));
    }
    ...
    list.forEach((inv,idx)=>{
      ...
      tr.innerHTML=`
        ...
        <td>${safeText(displayParty(inv))}</td>
  ...
  $("mv_xlsx").onclick=()=>exportExcel("invoice_moves.xlsx", last.map(inv=>({
    ...
    type:invTitle(inv.type), no:inv.no, party:displayParty(inv)||"", pay:inv.payMethod||"",
...
function initInvoicePage(viewId,type){
  ...
  async function run(){
    ...
    const filtered=list.filter(inv=>{
      ...
      if(!s) return true;
      return String(inv.no).includes(s) || String(displayParty(inv)).includes(s) || (inv.lines||[]).some(l=>String(l.barcode||"").includes(s) || String(l.name||"").includes(s));
    });
