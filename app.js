...
function mountInvReports(){
  ...
  itemSel.innerHTML = `<option value="all">كل المواد</option>` + ITEMS.map(i=>`<option value="${i.id}">${(i.code?("["+i.code+"] "):"")}${i.name||""}</option>`).join("");
  ...
  $("r_move").onclick=async ()=>{
    ...
    invs
      ...
      .forEach(inv=>{
        ...
        (inv.lines||[]).forEach(l=>{
          ...
          rows.push([baseDate, invTitle(inv.type), String(inv.no||inv.id||""), whId, it?.code||"", it?.name||"", fmt.format(qty), party]);
        });
      });

    lastRows = rows.map(r=>({date:r[0],type:r[1],no:r[2],warehouse:r[3],code:r[4],name:r[5],qty:r[6],party:r[7]}));
    ...
    setTable(["تاريخ","نوع","رقم","مستودع","كود","اسم","كمية(+/-)","طرف"],rows);
  };
  ...
  $("r_valuation").onclick=async ()=>{
    ...
    const rows=items.map(it=>{
      ...
      const val=qty*cost;
      totalValue += val;
      return [it.code||"",it.name||"",it.unit||"",fmt.format(qty),fmt.format(cost),fmt.format(val)];
    });

    lastRows = rows.map(r=>({code:r[0],name:r[1],unit:r[2],qty:r[3],cost:r[4],value:r[5]}));
    ...
    setTable(["كود","اسم","وحدة","كمية","كلفة/وحدة","قيمة"],rows);
  };
  ...
  $("r_bywh").onclick=async ()=>{
    ...
    const headers=["كود","اسم","وحدة", ...WAREHOUSES.map(w=>w.name||w.id), "الإجمالي"];
    const rows=items.map(it=>{
      let total=0;
      ...
      return [it.code||"",it.name||"",it.unit||"", ...cols, fmt.format(total)];
    });

    // export friendly
    lastRows = rows.map(r=>{
      const obj={code:r[0],name:r[1],unit:r[2]};
      WAREHOUSES.forEach((w,idx)=> obj["wh_"+(w.name||w.id)]=r[3+idx]);
