...
import { signInWithEmailAndPassword, signOut, onAuthStateChanged, createUserWithEmailAndPassword, getAuth } from "https://www.gstatic.com/firebasejs/12.9.0/firebase-auth.js";
...
const getVal=(id,def="")=>{ const el=$(id); return el? el.value : def; };
...
function computeTotals(){
  ...
  const discAmt=n(getVal("posDiscAmt"));
  const discPct=n(getVal("posDiscPct"));
...
function renderPreview(){
  ...
  const type=getVal("posType");
  const whName=WAREHOUSES.find(w=>w.id===getVal("posWh"))?.name||"";
  const pName=partyName(type,getVal("posParty"));
...
async function saveInvoice(){
  ...
  $("posWarn").innerText="";
  const type=getVal("posType");
  const whId=getVal("posWh");
  const pay=getVal("posPay");
  const pId=getVal("posParty");
  const note=getVal("posNote").trim();
  const {total,disc,net}=computeTotals();
...
function invoiceA4HTMLFrom(inv){
  ...
  const lines=(inv.lines||[]).map(l=>{
    ...
    const sum=qty*price;
    return `<tr><td>${safeText(l.code||"")}</td><td>${safeText(l.name||"")}</td><td>${fmt.format(qty)}</td><td>${safeText(l.unit||"")}</td><td>${fmt.format(price)}</td><td>${fmt.format(sum)}</td></tr>`;
  }).join("");
  ...
  <div class="pdoc invoice">
    <div class="pHead">
      ...
      <div class="pMeta">
        <div><b>رقم:</b> ${safeText(inv.no||"-")}</div>
        <div><b>تاريخ:</b> ${dateStr}</div>
        <div><b>الدفع:</b> ${payLabel(inv.payMethod)}</div>
      </div>
    </div>
    ...
    <div class="pMetaGrid">
      <div><b>الطرف:</b> ${safeText(inv.partyName||"")}</div>
      <div><b>المستودع:</b> ${safeText(whName)}</div>
      <div><b>ملاحظة:</b> ${safeText(inv.note||"")}</div>
    </div>

    <div class="tableWrap pTable">
      <table class="tbl">
        <thead><tr><th>كود</th><th>الصنف</th><th>الكمية</th><th>الوحدة</th><th>السعر</th><th>الإجمالي</th></tr></thead>
        <tbody>${lines||`<tr><td colspan="6" class="hint">لا توجد بنود</td></tr>`}</tbody>
      </table>
    </div>

    <div class="pTotals">
      <div class="pTot"><span>الإجمالي</span><b>${fmt.format(total)}</b></div>
      <div class="pTot"><span>خصم</span><b>${fmt.format(disc)}</b></div>
      <div class="pTot net"><span>الصافي</span><b>${fmt.format(net)}</b></div>
    </div>

    <div class="pFoot">
      <div class="pSign">
        <div class="pSignLbl">المستلم</div><div class="pSignLine"></div>
      </div>
      <div class="pSign">
        <div class="pSignLbl">المحاسب</div><div class="pSignLine"></div>
      </div>
      <div class="pSign">
        <div class="pSignLbl">المدير</div><div class="pSignLine"></div>
      </div>
    </div>
  </div>`;
}
...
function mountPOS(){
  ...
  $("posAdd").onclick=async ()=>{
    ...
    const type=getVal("posType");
    const whId=getVal("posWh");
    const itemId=getVal("posItem");
    const qty=Math.max(1,n(getVal("posQty")));
    const override=getVal("posPrice").trim();
...
function mountItems(){
  ...
  $("i_save").onclick=async ()=>{
    ...
    const name=getVal("i_name").trim(); if(!name) return $("i_msg").innerText="الاسم مطلوب.";
    const docRef=await addDoc(collection(db,"companies",COMPANY_ID,"items"),{
      code:getVal("i_code").trim(),name,unit:getVal("i_unit").trim(),
      price:Math.max(0,n(getVal("i_price"))),
      purchasePrice:Math.max(0,n(getVal("i_pprice"))),
      active:setValById("i_active", =="true",
      createdAt:serverTimestamp(),iso:iso()
    }));
    const oq=Math.max(0,n(getVal("i_oqty")));
    if(oq>0){
      const wh=getVal("i_wh");
    ...
    ["i_code","i_name","i_unit"].forEach(id=>setValById(id,"")); setValById("i_price", "0"); setValById("i_pprice", "0"); setValById("i_oqty", "0");
...
function mountCustomers(){
  ...
  $("c_save").onclick=async ()=>{
    ...
    const name=getVal("c_name").trim(); if(!name) return $("c_msg").innerText="الاسم مطلوب.";
    const ref=await addDoc(collection(db,"companies",COMPANY_ID,"customers"),{
      name,phone:getVal("c_phone").trim(),address:getVal("c_addr").trim(),note:getVal("c_note").trim(),
      active:true,createdAt:serverTimestamp(),iso:iso()
    });
    await audit("create","customer",ref.id,{name});
    ["c_name","c_phone","c_addr","c_note"].forEach(id=>setValById(id,""));
...
function mountSuppliers(){
  ...
  $("s_save").onclick=async ()=>{
    ...
    const name=getVal("s_name").trim(); if(!name) return $("s_msg").innerText="الاسم مطلوب.";
    const ref=await addDoc(collection(db,"companies",COMPANY_ID,"suppliers"),{
      name,phone:getVal("s_phone").trim(),address:getVal("s_addr").trim(),note:getVal("s_note").trim(),
      active:true,createdAt:serverTimestamp(),iso:iso()
    });
    await audit("create","supplier",ref.id,{name});
    ["s_name","s_phone","s_addr","s_note"].forEach(id=>setValById(id,""));
...
function mountAccounts(){
  ...
  $("a_save").onclick=async ()=>{
    ...
    const code=getVal("a_code").trim(); const name=getVal("a_name").trim();
    if(!code||!name) return $("a_msg").innerText="الكود والاسم مطلوبان.";
    await setDoc(doc(db,"companies",COMPANY_ID,"accounts",code),{
      code,name,parent:getVal("a_parent").trim(),level:n(getVal("a_level"))||1,type:getVal("a_type"),
      allowPost:setValById("a_post", =="true",updatedAt:serverTimestamp(),iso:iso()
    },{merge:true}));
...
function mountMap(){
  ...
  const set=(id,val)=>setValById(id, val||"");
  ...
  $("m_save").onclick=async ()=>{
    ...
    MAP={
      cashAccount:getVal("m_cash").trim(),
      vodafoneAccount:getVal("m_vod").trim(),
      instaAccount:getVal("m_insta").trim(),
      salesAccount:getVal("m_sales").trim(),
      salesReturnAccount:getVal("m_sret").trim(),
      purchaseAccount:getVal("m_pur").trim(),
      purchaseReturnAccount:getVal("m_pret").trim(),
      arControl:getVal("m_ar").trim(),
      apControl:getVal("m_ap").trim(),
...
function mountInvReports(){
  ...
  setValById("r_from", f.toISOString().slice(0,10));
  setValById("r_to", d.toISOString().slice(0,10));
  ...
  $("r_move").onclick=async ()=>{
    ...
    const from=getVal("r_from"), to=getVal("r_to"), wh=whSel.value;
...
function mountOpening(){
  ...
  const d=new Date(); setValById("o_date", d.toISOString().slice(0,10));
  ...
  $("o_add").onclick=()=>{
    const acc=getVal("o_acc");
    const dr=Math.max(0,n(getVal("o_dr")));
    const cr=Math.max(0,n(getVal("o_cr")));
    if(!acc || (dr<=0 && cr<=0)) return;
    const a=ACCOUNTS.find(x=>String(x.code)===String(acc));
    lines.push({account:acc,name:`${a.code} - ${a.name}`,debit:dr,credit:cr,memo:getVal("o_memo").trim()});
    setValById("o_dr", "0"); setValById("o_cr", "0"); setValById("o_memo", "");
    render();
  };
  ...
  $("o_save").onclick=async ()=>{
    ...
    const locked=parseLockedUntil();
    const vDate=new Date(getVal("o_date")+"T00:00:00");
    ...
    try{
      const saved = await runTransaction(db, async (tx)=>{
        ...
        const ono=`OPEN-${no}`;
        const odate=new Date(getVal("o_date")+"T00:00:00").toISOString();
        const onote=getVal("o_note").trim()||"قيد افتتاحي";
        const ref=doc(collection(db,"companies",COMPANY_ID,"journalEntries"));
        tx.set(ref,{
          ts:serverTimestamp(),
          date:odate,
  ...
  $("o_print").onclick=()=>{
    const data = LAST_DOC.opening || {
      no:"OPEN-preview",
      date:new Date(getVal("o_date")+"T00:00:00").toISOString(),
      note:getVal("o_note").trim()||"قيد افتتاحي",
      lines: (function(){
...
function mountReports(){
  ...
  $("view_cogs").innerHTML=reportShell("تقرير الربحية (مبيعات مقابل COGS)","cogs");
...
async function runTrial(){
  const from=getVal("trial_from"), to=getVal("trial_to");
...
async function runCust(){
  const from=getVal("cust_from"), to=getVal("cust_to");
  const party=getVal("cust_party");
  ...
  $("cust_title").innerText=`كشف حساب عميل: ${partyName} (${from} → ${to})`;
  renderReport("cust",{
    title:"كشف حساب عميل",
    columns:[
      {k:"date",label:"تاريخ"},
    ...
    rows,
    hint:"القيمة (+) مبيعات، والقيمة (-) مرتجعات."
  });
}

async function runSup(){
  const from=getVal("sup_from"), to=getVal("sup_to");
  const party=getVal("sup_party");
  ...
  $("sup_title").innerText=`كشف حساب مورد: ${partyName} (${from} → ${to})`;
  renderReport("sup",{
    title:"كشف حساب مورد",
    columns:[
      {k:"date",label:"تاريخ"},
    ...
    rows,
    hint:"القيمة (+) مشتريات، والقيمة (-) مرتجعات مشتريات."
  });
}

async function runBalances(){
  const from=getVal("bal_from"), to=getVal("bal_to");
...
async function runCogs(){
  const from=getVal("cogs_from"), to=getVal("cogs_to");
  const sales=[...(await loadInvoices("sale")),...(await loadInvoices("sale_return"))];
  const byItem={};
  ...
  $("cogs_title").innerText=`تقرير الربحية (${from} → ${to})`;
...
async function runCash(){
  const from=getVal("cash_from"), to=getVal("cash_to");
  const acc=getVal("cash_acc");
  const entries=await loadJournal();
...
function mountUsers(){
  ...
  $("u_create").onclick=async ()=>{
    ...
    const email=getVal("u_email").trim().toLowerCase();
    const pass=getVal("u_pass");
    const name=getVal("u_name").trim();
    const role=getVal("u_role");
    const canOverride=setValById("u_override", =="true");
    ...
    try{
      ...
      ["u_email","u_pass","u_name","u_note"].forEach(id=>setValById(id,""));
      await refreshUsers();
    }catch(e){
  ...
  $("u_update").onclick=async ()=>{
    if(!can(["admin"])) return $("u_msg").innerText="للأدمن فقط.";
    const uid=getVal("u_pick"); if(!uid) return;
    const role=getVal("u_role2");
    const canOverride=setValById("u_override2", =="true");
    const isActive=setValById("u_active2", =="true");
    await setDoc(doc(db,"companies",COMPANY_ID,"users",uid),{
...
async function refreshUsers(){
  ...
  if(pick){
    ...
    pick.onchange=()=>{
      ...
      if(!r) return;
      setValById("u_role2", r.role||"viewer");
      setValById("u_override2", r.canOverrideLock?"true":"false");
      setValById("u_active2", r.isActive?"true":"false");
    };
...
function mountCompany(){
  ...
  $("co_save").onclick=async ()=>{
    ...
    const data={
      name:(getVal("co_name")||"").trim(),
      address:(getVal("co_addr")||"").trim(),
      city:(getVal("co_city")||"").trim(),
      phoneSales:(getVal("co_phone")||"").trim(),
      whatsapp:(getVal("co_whats")||"").trim(),
      facebook:(getVal("co_fb")||"").trim(),
      telegram:(getVal("co_tg")||"").trim(),
      website:(getVal("co_web")||"").trim(),
      mapsLink:(getVal("co_maps")||"").trim(),
      footerNote:(getVal("co_note")||"").trim(),
      returnPolicy:(getVal("co_policy")||"").trim(),
      updatedAt:serverTimestamp(),iso:iso()
    };
  ...
  $("co_switchCompany").onclick=()=>{
    ...
    const id=getVal("co_selectCompany").trim();
    if(!id) return;
  ...
  $("co_createCompany").onclick=async ()=>{
    if(!can(["admin"])) return $("co_companyMsg").innerText="للأدمن فقط.";
    const id=(getVal("co_newCompanyId")||"").trim();
  ...
  $("co_wipeAll").onclick=async ()=>{
    ...
    const v=(getVal("co_wipeConfirm")||"").trim().toUpperCase();
...
$("btnLogin").onclick=async ()=>{
  ...
  const rawUser=getVal("loginEmail").trim();
  const pass=getVal("loginPassword");

  // دعم تسجيل الدخول باسم المستخدم (Admin) كما طلبت
  // إذا كتب المستخدم "Admin" بدون بريد، سنحوّله لبريد افتراضي وننشئه تلقائياً عند أول تشغيل.
  let email = rawUser;
  if(email && !email.includes("@")){
    // اسم مستخدم -> بريد افتراضي
    email = `${email.toLowerCase()}@local.erp`;
  }

  try{
    await signInWithEmailAndPassword(auth,email,pass);
  }catch(e){
    // إنشاء Admin تلقائياً إذا لم يكن موجوداً (مرة واحدة)
    const isAdminSeed = (rawUser.toLowerCase()==="admin" && pass==="Admin123");
    const msg = (e && (e.code||e.message||""))+"";
    if(isAdminSeed && (msg.includes("auth/user-not-found") || msg.includes("user-not-found"))){
      try{
        // إنشاء المستخدم ثم تسجيل الدخول
        await createUserWithEmailAndPassword(auth, email, pass);
        await signInWithEmailAndPassword(auth, email, pass);
        return;
      }catch(e2){
        $("loginError").innerText="تعذر إنشاء مستخدم Admin تلقائياً. راجع إعدادات Firebase Auth (Email/Password) وتأكد أنها مفعّلة.";
        return;
      }
    }
...
onAuthStateChanged(auth, async (user)=>{
  ...
  mountCompany();
  mountAudit();
  // صفحات العمليات (POS / فواتير / سندات / تقارير)
  mountInvoicePages();
  mountVouchers();
  mountOpening();
  mountInvReports();
