...
function canDo(key){
  ...
  const r=role();
  const defaults={
    cashier:{pos:true, sales:true, salesReturn:true, customers:true, suppliers:true, vouchers:true, reports:true},
...
async function loadInvoices(type){
  ...
  snap.forEach(d=>{ 
    ...
    // Older builds saved party fields as customerName/supplierName (and sometimes customerId/supplierId).
    // This app expects partyId/partyName.
    if(!obj.partyId){
      obj.partyId = obj.customerId || obj.supplierId || obj.partyId || "";
    }
    if(!obj.partyName){
      obj.partyName = obj.customerName || obj.supplierName || obj.party || obj.partyName || "";
    }
    // Keep also customerName/supplierName populated for reports that may use them.
    if((obj.type==="sale"||obj.type==="sale_return") && !obj.customerName && obj.partyName){
      obj.customerName = obj.partyName;
    }
    if((obj.type==="purchase"||obj.type==="purchase_return") && !obj.supplierName && obj.partyName){
      obj.supplierName = obj.partyName;
    }
...
function invParty(inv){
  return inv?.partyName || inv?.customerName || inv?.supplierName || inv?.party || "";
}

function getPartyDisplay(type, inv){
  if(!inv) return "";
  const direct = inv.partyName || inv.customerName || inv.supplierName || inv.party || "";
  if(direct) return direct;
  const pid = String(inv.partyId || inv.customerId || inv.supplierId || "");
  if(!pid) return "";
  return partyName(type, pid) || "";
}
  ...
  <div class="pdoc invoice invoicePro">
    ...
    <div class="invPartyRow">
      <div class="invPartyRight">
        <div class="invPartyLine"><span>السيد:</span> <b>${safeText(getPartyDisplay(inv.type,inv)||"")}</b></div>
...
function invoiceReceiptHTMLFrom(inv){
  ...
  <div class="receipt">
    ...
    <div class="rSmall">الطرف: ${safeText(getPartyDisplay(inv.type,inv)||"")}</div>
...
function mountInvoiceMoves(){
  ...
  const load=async ()=>{
    ...
    if(q){
      list=list.filter(inv=>String(inv.no).includes(q) || String(getPartyDisplay(inv.type,inv)||"").includes(q));
    }
    ...
    list.forEach((inv,idx)=>{
      ...
      tr.innerHTML=`
        ...
        <td>${safeText(getPartyDisplay(type,inv)||"")}</td>
  ...
  $("mv_xlsx").onclick=()=>exportExcel("invoice_moves.xlsx", last.map(inv=>({
    ...
    type:invTitle(inv.type), no:inv.no, party:getPartyDisplay(type,inv)||"", pay:inv.payMethod||"",
...
function initInvoicePage(viewId,type){
  ...
  async function run(){
    ...
    const filtered=list.filter(inv=>{
      ...
      if(!s) return true;
      return String(inv.no).includes(s) || String(getPartyDisplay(type,inv)||"").includes(s) || (inv.lines||[]).some(l=>String(l.barcode||"").includes(s) || String(l.name||"").includes(s));
    });
    ...
    lastRows=filtered.map(inv=>({
      date:String(inv.date||"").slice(0,19).replace("T"," "),
      no:inv.no,
      party:getPartyDisplay(type,inv)||"",
      pay:inv.payMethod||"",
      net:n(inv.net),
  ...
  $("r_move").onclick=async ()=>{
    ...
    invs
      ...
      .forEach(inv=>{
        ...
        const party = getPartyDisplay(inv.type,inv) || "";
        (inv.lines||[]).forEach(l=>{
    ...
    const rows = sales
      ...
      .map(inv=>{
        ...
        return {
          ...
          party:(getPartyDisplay(inv.type,inv)||""),
          qty,
          revenue,
          cogs,
          profit,
...
function defaultPermsForRole(r){
  ...
  if(r==="cashier") return {pos:true,sales:true,salesReturn:true,customers:true,suppliers:true,vouchers:true,reports:true,invReports:true,cogs:true};
