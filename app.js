...
import { signInWithEmailAndPassword, signOut, onAuthStateChanged, createUserWithEmailAndPassword, getAuth, signOut as signOutAny } from "https://www.gstatic.com/firebasejs/10.12.5/firebase-auth.js";
...
// IMPORTANT: Firebase Auth/Firestore do NOT work reliably when opening the app via file://
// The user must run the app over https (GitHub Pages) or http://localhost (local server)
try{
  if(location.protocol === 'file:'){
    const msg = "⚠️ لا يمكن تشغيل النظام مباشرة من ملف (file://).\n"+
      "ارفع المشروع على GitHub Pages أو افتحه عبر سيرفر محلي (http://localhost) ليعمل Firebase.";
    console.warn(msg);
    // show message on login screen if available
    setTimeout(()=>{
      try{ const el=document.getElementById('loginError'); if(el) el.innerText = msg; }catch(e){}
    }, 50);
  }
}catch(e){}

function authErrorAR(err){
  const code = err?.code || "";
  // Map common Firebase Auth errors to Arabic actionable messages
  const map={
    ...
    "auth/network-request-failed":"مشكلة اتصال بالإنترنت. تحقق من الشبكة.",
    "auth/operation-not-allowed":"طريقة تسجيل الدخول (Email/Password) غير مفعلة في Firebase.",
    "auth/unauthorized-domain":"الدومين غير مسموح داخل Firebase.\nأضف الدومين في: Firebase Console → Authentication → Settings → Authorized domains.",
    "auth/invalid-credential":"بيانات الدخول غير صحيحة.",
    "auth/invalid-origin":"الأصل غير مسموح (عادةً عند تشغيل file:// أو دومين غير مضاف).",
  };
  const base = map[code] || "تعذر تسجيل الدخول.";
  return code ? `${base}\n(${code})` : base;
}
...
async function loadInvoices(type){
  ...
  snap.forEach(d=>{ 
    ...
    if(!type || obj.type===type) list.push(obj);
  });
  return list;
}
...
function invoicePartyDisplay(inv){
  // توحيد الاسم عبر الإصدارات (partyName / customerName / supplierName)
  const t=String(inv?.type||"");
  const pid=String(inv?.partyId||"");
  return safeText(inv?.partyName || inv?.customerName || inv?.supplierName || partyName(t,pid) || "");
}
  ...
  try{
    ...
    if(!EDITING_INVOICE_ID){
      ...
      result = await runTransaction(db, async (tx)=>{
        ...
        tx.set(invRef,{
          payMethod:pay,
          partyId:pId,partyName:partyName(type,pId),
  ...
  <div class="pdoc invoice invoicePro">
    ...
    <div class="invPartyRow">
      <div class="invPartyRight">
        <div class="invPartyLine"><span>السيد:</span> <b>${invoicePartyDisplay(inv)}</b></div>
...
function invoiceReceiptHTMLFrom(inv){
  ...
  <div class="receipt">
    ...
    <div class="rSmall">الطرف: ${invoicePartyDisplay(inv)}</div>
...
function voucherPrintA4HTMLFrom(kind, data){
  ...
  const party = safeText(
    data?.partyName ||
    (data?.lines||[]).slice(1).find(l=> (n(l.debit)>0 || n(l.credit)>0))?.name ||
    (data?.lines||[]).slice(1).find(l=> (n(l.debit)>0 || n(l.credit)>0))?.account ||
    ""
  );
...
function mountInvoiceMoves(){
  ...
  const load=async ()=>{
    ...
    if(q){
      list=list.filter(inv=>String(inv.no).includes(q) || String(invoicePartyDisplay(inv)).includes(q));
    }
  ...
  $("mv_xlsx").onclick=()=>exportExcel("invoice_moves.xlsx", last.map(inv=>({
    ...
    type:invTitle(inv.type), no:inv.no, party:invoicePartyDisplay(inv), pay:inv.payMethod||"",
...
function initInvoicePage(viewId,type){
  ...
  async function run(){
    ...
    const filtered=list.filter(inv=>{
      ...
      if(!s) return true;
      return String(inv.no).includes(s) || String(invoicePartyDisplay(inv)).includes(s) || (inv.lines||[]).some(l=>String(l.barcode||"").includes(s) || String(l.name||"").includes(s));
    });
...
function voucherHTML(title, kind){
  ...
  <div class="card">
    ...
    <div class="row3">
      ...
      <div class="field"><label>الطرف</label><input id="${kind}_party" placeholder="اختياري"></div>
    </div>
    <div class="row2">
      <div class="field"><label>البيان</label><input id="${kind}_note" placeholder="اختياري"></div>
      <div class="field"><label></label><div class="hint">يظهر (الطرف) في الطباعة ويومية الصندوق.</div></div>
    </div>
...
function initVoucher(kind){
  ...
  // الطرف (اختياري): اسم طرف السند يظهر في الطباعة والتقارير
  const partyInput = $(kind+"_party");
  if(partyInput){
    partyInput.placeholder = partyInput.placeholder || "مثال: عميل/مورد/حساب";
  }
  ...
  $(kind+"_save").onclick=async ()=>{
    ...
    try{
      const saved = await runTransaction(db, async (tx)=>{
        ...
        const partyLine = lines.slice(1).find(l=> (kind==="receipt" ? n(l.credit)>0 : n(l.debit)>0) );
        const partyAccount = partyLine?.account || "";
        const partyNameAuto = partyLine?.name || "";
        const partyName = (($(kind+"_party")?.value||"").trim()) || partyNameAuto;
        ...
        return { kind, no:vno, date:vdate, note:vnote, partyAccount, partyName,
          lines:lines.map(l=>({account:l.account,name:l.name,debit:n(l.debit),credit:n(l.credit),memo:l.memo||""})) };
      });
...
async function runCash(){
  ...
  const entries=await loadJournal();
  // map invoiceId -> partyName (لإظهار الطرف حتى للقيود القديمة)
  let invPartyMap={};
  try{
    const invs=await loadInvoices();
    invPartyMap = Object.fromEntries((invs||[]).map(i=>[i.id, (i.partyName||partyName(i.type||"sale", i.partyId||""))]));
  }catch(_){ invPartyMap={}; }
  ...
  entries.forEach(e=>{
    ...
    (e.lines||[]).forEach(l=>{
      ...
      if(d < from) opening += delta;
        ...
        const party = (e.partyName||"") || (e.invoiceId ? (invPartyMap[e.invoiceId]||"") : "");
        inRange.push({
...
async async function refreshCustomers(){
...
$("btnLogin").onclick=async ()=>{
  ...
  try{
    ...
    const msg = authErrorAR(e);
    $("loginError").innerText = msg;
    try{ showToast(msg,"danger",4500); }catch(err){}
